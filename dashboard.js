(() => {
  ...
  const storeName=store=>store==='ty'?'桃園':store==='jc'?'江翠':store==='sx'?'三峽':'整體';
  ...
  function updateNew() {
    ...
    $('#sms-store-progress').innerHTML=storeCodes.map(code=>{
      ...
      return '<div class="sms-store"><div class="sms-store-title"><strong>'+storeName(code)+'漾澤</strong><span>'+(v.t===null?'目標待補':pct(v.rate))+'</span></div>'+(v.t===null?'':'<div class="sms-track"><div class="sms-fill" style="width:'+Math.min((v.rate||0)*100,100)+'%"></div></div>')+'<div class="sms-store-meta"><span>實績 '+fmt(v.a)+' 人</span><span>'+(v.d===null?'':(v.d>=0?'超前':'落後')+' '+fmt(Math.abs(v.d)))+'</span></div></div>';
    }).join('');
  ...
  function updateConsumption() {
    ...
    $('#sms-consumption-stores').innerHTML=storeCodes.map(code=>{const v=consumptionTotal(active,code,cohort),share=combined>0?v.amount/combined:null;return `<div class="sms-store"><div class="sms-store-title"><strong>${storeName(code)}漾澤</strong><span>${pct(share)}</span></div><div class="sms-track"><div class="sms-fill ${code==='jc'?'sage':''}" style="width:${(share||0)*100}%"></div></div><div class="sms-store-meta"><span>${fmt(v.amount)} 元</span><span>消費 ${fmt(v.count)} 筆</span></div></div>`;}).join('');
  ...
  function updateMeta() {
    ...
    const budgetCopy=store==='all'?'全品牌 Meta 預算採「年度預算」的每月核定值。':'單店 Meta 預算採「Meta月度KPI」G 欄執行目標；未填月份不顯示預算與執行率。';
